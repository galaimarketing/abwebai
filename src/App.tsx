...
const App = () => {
  const [isInitialPromptSent, setInitialPromptSent] = useState(false);

  useEffect(() => {
    if (!isInitialPromptSent) {
      const initialPrompt =
        "You are only a marketing assistant and your name is ALA.";
      handleSubmit(initialPrompt, true);
  ...
  const handleSubmit = async (
    prompt: string,
    initialPrompt: boolean = false
  ) => {
    if (!prompt.trim().length || initialPrompt) {
      return; // Return early if it's an initial prompt or an empty message
    }
